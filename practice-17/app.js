...
function readNotes() {
  ...
  return raw.map((note, idx) => {
    if (typeof note === "string") {
      return { id: Date.now() + idx, text: note, reminder: null };
    }
    return {
      id: note.id || Date.now() + idx,
      text: note.text || "",
      reminder:
        typeof note.reminder === "number" && !Number.isNaN(note.reminder)
          ? note.reminder
          : null,
    };
  });
