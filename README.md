...
| `practice-19` | Express :3000 + PostgreSQL (драйвер `pg`). CRUD `/api/users` (POST, GET, GET/:id, PATCH, DELETE). Сущность User: `first_name`, `last_name`, `age`, `created_at`/`updated_at` (BIGINT, unix-мс, проставляются сервером). Таблица `users` создаётся при старте (`CREATE TABLE IF NOT EXISTS`). Конфиг через `.env` (`DATABASE_URL` или `PG*`-переменные). Запуск: создать БД (например `createdb practice19`), скопировать `.env.example` → `.env` и подставить пароль, `npm install && npm start`. |

**Стек:** HTML, SCSS, React, Vite, Axios, Node.js, Express, CORS, swagger-jsdoc, swagger-ui-express, multer, bcrypt, jsonwebtoken, Socket.IO, Service Worker/PWA, Web Push (VAPID), PostgreSQL (`pg`), dotenv.
